...
__all__ = ['SimpleProfilerPostCallback', 'SimpleProfilerCallback', 'SimpleProfilerPostCallback']
...
# Cell
class SimpleProfilerPostCallback(Callback):
    "Pair with `SimpleProfilerCallback` to capture accurate end times. Removes itself after training is over."
    order,remove_on_fetch = Recorder.order-1,True
    def __init__(self):
        if not hasattr(self.learn, 'simple_profiler'):
            raise Warning(f'Requires `SimpleProfilerCallback` to work. Use Learner.simple_profile()')
        self.profiler = self.learn.simple_profiler
    ...
    def after_pred(self):
        if self.training: self._train_loss_start = time.monotonic()
        else: self._valid_loss_start = time.monotonic()
    ...
    def before_step(self):
        self.profiler._raw_time['backward'].append(time.monotonic() - self.profiler._backward_start)
    ...
    def after_fit(self):
        ...
        if not hasattr(self.learn, 'lr_finder'):
            self.profiler._display_report()
            self.learn.removed_cbs([SimpleProfilerCallback, SimpleProfilerPostCallback])